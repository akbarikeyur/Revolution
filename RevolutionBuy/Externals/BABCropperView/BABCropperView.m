...
static UIImage* BABCropperViewCroppedAndScaledImageWithCropRect(UIImage *image, CGRect cropRect, CGSize scaleSize, BOOL cropToCircle, BOOL transparent) {
    ...
    CGContextRef bitmap = CGBitmapContextCreate(NULL, scaleSize.width, scaleSize.height, 8, 0, colorspace, kCGImageAlphaPremultipliedLast | kCGBitmapByteOrder32Big);
...
- (void)updateScrollViewZoomScales {
    ...
    if(self.image) {
        ...
        BOOL allowsNegativeSpaceInCroppedImage = scaleBasedOnWidth < scaleBasedOnHeight ? NO : YES;
        
        CGFloat minimumZoomScaleBasedOnHeight = allowsNegativeSpaceInCroppedImage ? scaleBasedOnWidth : scaleBasedOnHeight;
        CGFloat minimumZoomScalescaleBasedOnWidth = allowsNegativeSpaceInCroppedImage ? scaleBasedOnHeight : scaleBasedOnWidth;
